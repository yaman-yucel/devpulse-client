...
app = typer.Typer(help="DevPulse Client - Activity tracking with pipeline architecture")
...
def enroll(
    ...
    enrollment_secret: str = typer.Option(..., help="Enrollment secret token from admin"),
    hostname: str = typer.Option(None, help="Device hostname (auto-detected if not provided)"),
):
    """Enroll this device."""
    setup_logging()
    from loguru import logger
